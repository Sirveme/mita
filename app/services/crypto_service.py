...
# --- Cifrado genérico (credenciales de integraciones, etc.) ---
def encrypt(plain: str | None) -> str | None:
    """Cifra cualquier secreto. None si no hay valor/llave."""
    if not plain:
        return None
    f = _fernet()
    if f is None:
        logger.warning("SOL_ENCRYPTION_KEY no configurada: el secreto NO se almacena (evita texto plano).")
        return None
    try:
        return f.encrypt(plain.encode("utf-8")).decode("utf-8")
    except Exception as e:
        logger.error("Error cifrando secreto: %s", e)
        return None


def decrypt(token: str | None) -> str | None:
    """Descifra un secreto. None si falla."""
    if not token:
        return None
    f = _fernet()
    if f is None:
